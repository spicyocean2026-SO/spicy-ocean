...
import { Search, Coffee, ArrowLeft, Minus, Plus, ShoppingCart, Trash2, Send, Loader2 } from 'lucide-react';
...
const TeaSnacksPage: React.FC = () => {
  const { teaSnacksItems, menuLoading, activeOrders, placeOrder, settings } = useRestaurant();
  ...
  const tsOrders = activeOrders.filter((o) => o.type === 'TEA_SNACKS');
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    const res = await placeOrder({ type: 'TEA_SNACKS', items: cart });
    setSending(false);
    if (res) { setCart([]); setMode('list'); }
  };

  const viewed = viewOrderId ? tsOrders.find((o) => o.orderId === viewOrderId) : null;
  if (viewed) {
      ...
      <div className="p-4 md:p-6 max-w-2xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Tea &amp; Snacks · {viewed.paymentStatus}</p>
          </div>
        </div>
      ...
      <div className="flex flex-col h-[calc(100vh-48px)]">
        <div className="flex items-center gap-3 px-4 py-3 bg-card border-b border-border">
          ...
          <div className="flex-1">
            <h2 className="font-semibold text-foreground">New Tea &amp; Snacks</h2>
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-foreground flex items-center gap-2"><Coffee className="w-7 h-7 text-primary" /> Tea &amp; Snacks</h2>
          <p className="text-sm text-muted-foreground mt-1">Create and track tea &amp; snacks orders</p>
        </div>
