...
import { Search, ArrowLeft, Minus, Plus, ShoppingCart, Trash2, PackageOpen } from 'lucide-react';
...
const TakeAwayPage: React.FC = () => {
  const {
    menuItems, takeAwayOrders, createTakeAwayOrder,
    addItemToTakeAway, removeItemFromTakeAway, updateTakeAwayItemQuantity, settings,
  } = useRestaurant();
  ...
  const currentOrder = activeOrderId ? takeAwayOrders[activeOrderId] : null;
  const cartItems = currentOrder?.items || [];
  const cartTotal = cartItems.reduce((sum, i) => sum + i.menuItem.price * i.quantity, 0);

  const activeList = Object.entries(takeAwayOrders).filter(([, o]) => o.items.length > 0 || o.paymentStatus === 'pending');
      ...
      <div className="p-4 md:p-6 max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Create or continue a take away order</p>
          </div>
          <button
            onClick={() => {
              const id = createTakeAwayOrder();
              setActiveOrderId(id);
            }}
    ...
    <div className="flex flex-col h-[calc(100vh-48px)]">
      <div className="flex items-center gap-3 px-4 py-3 bg-card border-b border-border">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">{cartItems.length} items • Take Away</p>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <AnimatePresence mode="popLayout">
                {filteredItems.map(item => (
                  <FoodItemCard key={item.id} item={item} onAdd={(i) => addItemToTakeAway(activeOrderId, i)} />
                ))}
              </AnimatePresence>
            </div>
              ...
              {cartItems.length === 0 ? (
                ...
                <div key={item.menuItem.id} className="flex items-center gap-3 py-2 border-b border-border last:border-0">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => updateTakeAwayItemQuantity(activeOrderId, item.menuItem.id, item.quantity - 1)}
                  ...
                  <button onClick={() => removeItemFromTakeAway(activeOrderId, item.menuItem.id)} className="text-destructive p-1">
